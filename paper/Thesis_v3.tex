...
In Figure \ref{fig:densityGlobal} it is shown a density plot regarding the number of days that passed. We can see that the higher density is until 1250 days that passed. We can then see that the density increased again between 2500 and 5000 days. This is due to the fact that the allocation files contain allocations since 1982 while there are only BGP data files since 2001. To overcome this...

An important day for our analysis is February 3 2011, when the Internet Assigned Numbers Authority allocated the remaining /8 blocks to the Regional Internet Registries. Therefore we decided to analyze what happen before and after this date with regards to the number o days that took for a allocated prefix to be seen in routing. The results for before the date are shown in Figure \ref{fig:densityBefore}

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.35]{densityBefore.pdf}
\caption{densityBefore}
\label{fig:densityBefore}
\end{figure}

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.35]{densityAfter.pdf}
\caption{densityAfter}
\label{fig:densityAfter}
\end{figure}
