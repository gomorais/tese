...
def heatMapData(allocYear, heatMapDict, date):
	network = {}
	for x in xrange(1,221):		
		prefix = str(x) +".0.0.0"
		network[prefix] = set()
		heatMapDict[str(date)] = network

	for network in allocYear:
		result = re.search(r"\A\d{1,3}\.", network)
		if result:
			eight = result.group(0).strip(".")
			eight = eight + ".0.0.0"
			for prefix in allocYear[network]:
				if eight in heatMapDict[str(date)]:
					heatMapDict[str(date)][eight].add(prefix)
...
if __name__ == "__main__":
	...
	for f in allocFiles:
  	...
  	files = []
	for root, dirs, filenames in os.walk("./results/prefixMap24NewYear_newPeer"):
	...
	for date in prefixSeen:
		...
		heatMapData(allocYear, heatMapDict, date)
	...
	outputfile = open("./results/compareOriginAllocFiles_routeStatus_heatMapAll_24_new.csv", 'w')
	for date in heatMapDict:
		routingDate = str(date) + "-01-01"
		for network in heatMapDict[date]:	
			outputfile.write(str(network) + "," + str(len(heatMapDict[date][network])) + "," + routingDate + "\n")
	outputfile.close()
